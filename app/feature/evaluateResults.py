...
plt.title('Gap of tour from best known solution', fontsize=25)
plt.ylabel('Gap', fontsize=17)
plt.xlabel('Problem name', fontsize=17)
plt.legend(legend, prop={'family': 'monospace', 'size': 17}, title='Mean Gap', title_fontsize=15)
...
# Make leader board
columns = ['problemName', 'bestKnownSolution'] + gapAlgoNameList + algoResults
...
for index, row in leaderBoard.iterrows():
    ...
    content = [f"{algo}-{row[f'{algo}Result']}-{np.round(row[f'{algo}Gap'] * 100, 2)}%" for algo in algorithmNameList]
    content = np.array(content)[rankInd]
    leaderBoard.at[index, orders] = content

# remove unnecessary columns
leaderBoard.drop(columns=gapAlgoNameList + algoResults, inplace=True)
# Example about a value in order: 'ac-6780-0.0%'
for ord in orders:
    # make a list to save information about algorithm, distance solution and gap
    leaderBoard[ord] = leaderBoard[ord].str.split('-')
    # convert 6780 from string to integer
    leaderBoard[ord] = leaderBoard[ord].apply(
        lambda listValue: [listValue[0], int(listValue[1]), listValue[2]])
# Save leaderboard
